...
{
    public IParser Successor { get; private set; } = new FileDeleteCommandParser();
    ...
    public ICommand DefineCommand(string command)
    {
        ...
        if (command != null &&
            command.Contains("copy", StringComparison.Ordinal) && command.Contains("file", StringComparison.Ordinal))
        {
            return new FileCopyCommand(new FilePath(ParsedString.ToList()[2]), new FilePath(ParsedString.ToList()[3]));
        }
        else
        {
