...
{
    public IParser Successor { get; private set; } = new FileCopyCommandParser();
    ...
    public ICommand DefineCommand(string command)
    {
        ...
        if (command != null &&
            command.Contains("move", StringComparison.Ordinal) && command.Contains("file", StringComparison.Ordinal))
        {
            return new FileMoveCommand(new FilePath(ParsedString.ToList()[2]), new FilePath(ParsedString.ToList()[3]));
        }
        else
        {
